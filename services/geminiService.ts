...
export const generateSWOTAnalysis = async (userInput: UserInput): Promise<SWOTAnalysisResult> => {
  ...
  const buildPrompt = (userInput: UserInput) => `
    ...
    **REGRAS DE OURO:**
    ...
    - **Toda a sua análise deve se basear em informações vigentes e existentes no dia atual da pesquisa. Deixe isso claro na sua análise quando relevante.**
  ...
  try {
    return JSON.parse(jsonText) as SWOTAnalysisResult;
  } catch (e) {
    console.error("Falha ao analisar a resposta JSON:", jsonText);
    throw new Error("Recebido um formato inválido da IA.");
  }
};
